...
export default function DoctorRegisterScreen() {
  ...
  const { control, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { name: '', email: '', password: '', specialization: '' },
  });

  const onSubmit = async (data: FormData) => {
    setIsLoading(true);
    try {
      // Assuming register API exists for doctor
      await doctorAuthApi.register(data);
      Alert.alert('نجاح', 'تم استلام طلب انضمامك بنجاح! سيتم مراجعة حسابك', [
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.glassCard}>
              <View style={styles.headerContainer}>
                ...
                <Text style={[styles.title, { fontFamily: 'Cairo-Bold' }]}>انضمام كطبيب</Text>
                <Text style={[styles.subtitle, { fontFamily: 'Cairo-Regular' }]}>أدخل بياناتك لطلب الانضمام كطبيب جديد</Text>
              </View>

              <View style={styles.formContainer}>
                <View style={styles.fieldWrapper}>
                  <Text style={[styles.label, { fontFamily: 'Cairo-SemiBold' }]}>اسم الطبيب</Text>
                  <Controller
                    control={control}
                    name="name"
                  ...
                  />
                  {errors.name && <Text style={[styles.errorText, { fontFamily: 'Cairo-Regular' }]}>{errors.name.message}</Text>}
                </View>
                ...
                <View style={styles.fieldWrapper}>
                  <Text style={[styles.label, { fontFamily: 'Cairo-SemiBold' }]}>التخصص</Text>
                  <Controller
                    control={control}
                    name="specialization"
                    render={({ field: { onChange, onBlur, value } }) => (
                      <TextInput
                        style={[styles.input, { fontFamily: 'Cairo-Regular' }]}
                        placeholder="طبيب عام، أسنان، أطفال..."
                        placeholderTextColor="rgba(255,255,255,0.4)"
                        onBlur={onBlur}
                        onChangeText={onChange}
                        value={value}
                  ...
                  />
                  {errors.specialization && <Text style={[styles.errorText, { fontFamily: 'Cairo-Regular' }]}>{errors.specialization.message}</Text>}
                </View>
                ...
                >
                  {isLoading ? (
                    ...
                    <Text style={[styles.submitText, { fontFamily: 'Cairo-Bold' }]}>تقديم الطلب</Text>
                  )}
                </TouchableOpacity>
