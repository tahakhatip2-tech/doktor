...
export default function ChatRoomScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchMessages();
      // يمكن إضافة Polling هنا لتحديث الرسائل كل فترة لو أردت
      const interval = setInterval(fetchMessages, 10000); // 10 seconds polling
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setInputText(textToSend); // استعادة النص
    } finally {
      setIsSending(false);
    }
  };
...
const styles = StyleSheet.create({
  ...
  msgMe: { justifyContent: 'flex-start' }, // في العربية (RTL)، الـ start يعني اليمين
