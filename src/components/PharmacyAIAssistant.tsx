...
const CATEGORIES: Category[] = [
    {
        ...
        prompts: [
            { label: '📦 النواقص', prompt: 'أعطني قائمة بالأدوية التي قاربت على النفاد' },
            { label: '⚠️ قريبة الانتهاء', prompt: 'ما هي الأدوية التي ستنتهي صلاحيتها قريباً؟' },
            { label: '📈 الأكثر مبيعاً', prompt: 'ما هي أكثر الأدوية أو المنتجات مبيعاً هذا الأسبوع؟' },
        ],
    },
    {
        id: 'sales',
        ...
        prompts: [
            { label: '📊 مبيعات اليوم', prompt: 'أعطني ملخصاً لمبيعات اليوم وإجمالي الإيرادات في نقطة البيع' },
            { label: '💰 تحليل مالي', prompt: 'حلل الأداء المالي للصيدلية هذا الشهر وأعطني توقعات الأرباح' },
            { label: '🛒 تحليل الفواتير', prompt: 'ما هو متوسط قيمة الفاتورة اليوم؟' },
        ],
    },
    {
        id: 'advice',
        ...
        prompts: [
            { label: '💡 زيادة المبيعات', prompt: 'أعطني 5 نصائح لزيادة مبيعات منتجات العناية بالبشرة' },
            { label: '👥 إدارة الطاقم', prompt: 'نصائح لإدارة ورديات الصيادلة والمساعدين في الصيدلية' },
            { label: '⭐ ولاء العملاء', prompt: 'كيف أبني برنامج ولاء فعال لعملاء الصيدلية؟' },
        ],
    },
    {
        id: 'customers',
        ...
        prompts: [
            { label: '📋 صرف وصفة', prompt: 'أريد صرف وصفة لمريض جديد، ما هي الخطوات؟' },
            { label: '💬 تواصل مع مريض', prompt: 'أريد إرسال تذكير بموعد الدواء لمرضى الأمراض المزمنة' },
            { label: '🔍 حساب عميل', prompt: 'كيف أفتح حساب ذمم لعميل دائم؟' },
        ],
    },
    {
        id: 'settings',
        ...
        prompts: [
            { label: '⚙️ إعدادات الصيدلية', prompt: 'ما الإعدادات الحالية للصيدلية وكيف يمكنني تحسينها؟' },
            { label: '🖨️ الطابعات والباركود', prompt: 'كيف أضبط طابعة الفواتير وقارئ الباركود؟' },
        ],
    },
];
...
// ─── Main Component ────────────────────────────────────────────────────────────
export default function PharmacyAIAssistant() {
    ...
    const doctorName = settings?.pharmacy_name || 'الصيدلاني';
    const clinicName = settings?.pharmacy_name || 'الصيدلية';
...
متوسط التقييم: ${s.avgRating ?? 'غير محدد'}`.trim();
        ...
        return `أنت مساعد ذكي احترافي متقدم، اسمك "مساعد الصيدلية الذكي من Doctor Jo"، تعمل لصالح:
- الصيدلاني: د. ${doctorName}
- الصيدلية: ${clinicName}
- التاريخ/الوقت الحالي: ${today} - ${now.toLocaleTimeString('ar-JO')}

══════════════════════════════
أنت خبير في إدارة الصيدليات والمبيعات والمخزون الدوائي.
مهمتك مساعدة الصيدلاني في تنظيم عمله، إعطاء نصائح لزيادة المبيعات، وتحليل البيانات.

📜 قواعد الرد:
1. أجب دائماً بالعربية الفصحى المبسطة، وكن واضحاً ومباشراً وبأسلوب احترافي وودود.
2. استخدم الإيموجي المناسبة لتكون الردود حيوية ومريحة للعين.
3. قدم دائماً نصائح وإرشادات ذات قيمة حقيقية في إدارة الصيدليات (تسويق، مبيعات، جرد، ترتيب الرفوف، نصائح تجميلية، التعامل مع المرضى).
4. نظراً لأنك غير متصل حالياً بنظام المخزون والمبيعات الحي، إذا طُلب منك أرقام أو تقارير، قم بتوفير "قوالب تقارير احترافية" أو نصائح عامة بكيفية استخراجها، أو قدم بيانات افتراضية توضيحية مع التنويه على ذلك.
5. كن استباقياً واقترح أفكاراً لزيادة الأرباح وتقليل الهدر الدوائي.
6. التزم بدورك كمساعد إداري وداعم فني للمبيعات والتنظيم، وتجنب صرف وصفات طبية بشكل مباشر بدون استشارة الصيدلاني.`;
    ...
    const renderActionBlock = (msg: Message) => {
        ...
        const statusConfig = {
            ...
            executing: { bg: 'bg-emerald-50 border-emerald-200', icon: <Loader2 className="h-4 w-4 text-emerald-500 animate-spin" />, label: 'جاري التنفيذ...' },
        ...
        <>
                ...
                {!isOpen && (
                    <motion.button
                        ...
                        className="fixed bottom-24 left-4 z-50 flex items-center justify-center h-14 w-14
                            rounded-full bg-emerald-600 text-white shadow-xl shadow-emerald-500/40 
                            hover:shadow-emerald-500/60 transition-all duration-300 group"
                    >
                        {/* Outer pulse */}
                        <span className="absolute -inset-1.5 rounded-full animate-ping bg-emerald-400/30 pointer-events-none" />
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        {/* ══ Header ══════════════════════════════════════════ */}
                        <div className="flex-shrink-0 bg-gradient-to-l from-emerald-700 via-emerald-600 to-sky-600 px-4 py-3.5 shadow-md z-10">
                            ...
                            <>
                                    ...
                                    <AnimatePresence initial={false}>
                                        {messages.map(msg => (
                                            ...
                                            >
                                                {/* Avatar */}
                                                {msg.role === 'assistant' && (
                                                    <div className="flex-shrink-0 h-8 w-8 rounded-full bg-emerald-600 flex items-center justify-center mt-0.5 shadow-sm">
                                                ...
                                                <div className={`flex-1 ${msg.role === 'user' ? 'flex flex-col items-end' : ''}`}>
                                                    ...
                                                    <div className={`relative group max-w-[92%] ${
                                                        msg.role === 'user'
                                                            ? 'bg-emerald-600 text-white rounded-2xl rounded-tl-sm px-4 py-2.5'
                                                            ...
                                                            <div className="mt-3 flex flex-wrap gap-1.5">
                                                                {msg.quickReplies.map((qr, i) => (
                                                                    <button
                                                                        ...
                                                                        disabled={isLoading}
                                                                        className="px-2.5 py-1 rounded-lg bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300 text-[11px] font-bold hover:bg-teal-100 active:scale-95 transition-all disabled:opacity-40"
                                                                    >
                                                                        {qr}
                                                                    </button>
                                                                ))}
                                        ...
                                        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2">
                                            ...
                                            <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 px-4 py-3 rounded-2xl rounded-tr-sm shadow-sm flex items-center justify-center">
                                                <div className="flex gap-1.5 items-center">
                                                    {[0, 150, 300].map(delay => (
                                                        <span key={delay} className="h-2 w-2 rounded-full bg-emerald-400 animate-bounce" style={{ animationDelay: `${delay}ms` }} />
                                                    ))}
                                                </div>
                                            </div>
                                        </motion.div>
                                ...
                                <div className="flex-shrink-0 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-3 z-10 shadow-[0_-4px_15px_-5px_rgba(0,0,0,0.05)]">
                                    <div className="flex items-end gap-2 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-600 px-3 py-2 shadow-sm
                                        focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-400/20 transition-all">
                                        
                                        {/* Attachment / Actions button (Mockup for UX) */}
                                        <button
                                            type="button"
                                            className="flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all mb-0.5"
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`flex-shrink-0 h-9 w-9 rounded-full flex items-center justify-center text-white mb-0.5 transition-all
                                                ${input.trim() && !isLoading 
                                                    ? 'bg-emerald-600 hover:bg-emerald-700 shadow-md shadow-emerald-500/30 active:scale-95' 
                                    ...
                                    <div className="flex items-center justify-between mt-2 px-1">
                                        <button
                                            ...
                                            className="flex items-center gap-1 text-[10px] text-emerald-600 hover:text-emerald-700 font-bold bg-emerald-50 px-2 py-1 rounded-md"
                                        >
