...
export default function PharmacyLayout() {
    ...
    const { pharmacy, signOut, loading } = usePharmacyAuth(true);
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC]">
            ...
            <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-white/70 backdrop-blur-3xl shadow-sm transition-all duration-300">
                <div className="w-full flex h-16 items-center justify-between px-4 sm:px-6 mx-auto max-w-7xl">
                    ...
                    <div className="flex lg:hidden items-center justify-between w-full gap-2 min-w-0" dir="rtl">
                                    ...
                                    <button className="relative group outline-none active:scale-95 transition-all duration-300">
                                        <div className="relative">
                                            ...
                                            <div className="relative h-11 w-11 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white overflow-hidden shadow-xl border-2 border-white/50">
                                                {pharmacy?.avatar ? (
                                                    <img
                                                        src={getAvatarSrc(pharmacy.avatar)}
                                                        alt="Avatar"
                                                        className="h-full w-full object-cover"
                                                    />
                                                ) : (
                                                    <span className="font-bold text-lg">
                                                        {pharmacy?.name?.charAt(0) || 'ص'}
                                                    </span>
                                                )}
                                            </div>
                        ...
                        <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
                            <div className="flex flex-col items-end leading-none min-w-0">
                                ...
                                <p className="text-[7px] font-bold uppercase tracking-wider bg-gradient-to-r from-orange-500 to-blue-600 bg-clip-text text-transparent truncate w-full text-right">Pharmacy Portal</p>
                            </div>
                    ...
                    <div className="hidden lg:flex items-center justify-between w-full">
                        ...
                        <Link to="/pharmacy/dashboard" className="flex items-center gap-2 outline-none group">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-56 mt-2 p-2 rounded-2xl shadow-2xl border-blue-50 pb-2">
                                    <DropdownMenuLabel className="px-3 py-2">
                                        <p className="text-sm font-black text-blue-900">{pharmacy.name}</p>
                                        <p className="text-[10px] text-blue-600 font-bold" dir="ltr">{pharmacy.email}</p>
                                    </DropdownMenuLabel>
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem onClick={() => navigate('/pharmacy/profile')} className="rounded-xl py-2 cursor-pointer font-bold text-sm">
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50">
                <nav className="h-[72px] bg-white/95 dark:bg-black/90 backdrop-blur-3xl border-t-2 border-teal-500 shadow-[0_-15px_60px_rgba(0,0,0,0.1)] flex justify-between items-center px-1 pb-1">
                    {navItems.map((item, index) => {
                        const Icon = item.icon;
                        const isTrulyActive = location.pathname === item.path ||
                            (item.path === '/pharmacy/dashboard' && location.pathname === '/pharmacy') ||
                            (item.path !== '/pharmacy/dashboard' && location.pathname.startsWith(item.path));
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={cn(
                                    ...
                                    isMain
                                        ? "h-14 w-14 rounded-full bg-gradient-to-tr from-teal-600 via-teal-500 to-teal-400 shadow-[0_8px_25px_rgba(20,184,166,0.4)] border-4 border-white dark:border-zinc-900 active:scale-95"
                                        : "p-2 rounded-xl transition-all duration-300",
                                    !isMain && isTrulyActive && "bg-teal-50 dark:bg-teal-500/10 border-2 border-teal-500/50 shadow-sm"
                                )}>
                                    {isMain && (
                                    ...
                                    <Icon
                                        className={cn(
                                            ...
                                            isMain
                                                ? "h-7 w-7 text-white"
                                                : isTrulyActive ? "h-5 w-5 text-teal-600" : "h-4.5 w-4.5 text-blue-700/70"
                                        )}
                                    />
                                </div>
                                ...
                                {!isMain ? (
                                    <span className={cn(
                                        ...
                                        isTrulyActive ? "text-teal-600" : "text-blue-900/60"
                                    )}>
                                        {item.label}
                                    </span>
                                ) : (
                                    <span className="text-[9px] font-extrabold mt-0.5 text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full shadow-sm">
                                        {item.label}
                                    </span>
                                )}
                                ...
                                {!isMain && isTrulyActive && (
                                    <div className="absolute -bottom-0.5 h-1 w-1 bg-teal-600 rounded-full" />
                                )}
                            </Link>
                        );
                    })}
