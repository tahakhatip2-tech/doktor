...
export default function PatientOffers() {
    ...
    const handleShare = (offer: any) => {
        ...
        const logo = offer.isSponsored ? offer.sponsorLogo : offer.user?.clinic_logo;
        const logoUrl = logo ? logoSrc(logo) : '';
        
        // Formatting for WhatsApp and other native share targets
        let shareText = `📢 *${offer.title}*\n👨‍⚕️ من: ${publisherName}\n\n${offer.content}\n\n🌐 تطبيق Doctor JO للرعاية الصحية`;
        
        if (logoUrl) {
            shareText += `\n🖼️ شعار العيادة: ${logoUrl}`;
        }
                                        ...
                                        <div className="flex items-center border-t border-slate-100 bg-slate-50/80 overflow-hidden relative z-20">
                                            ...
                                            {offer.isSponsored && offer.sponsorPhone ? (
                                                <a
                                                    ...
                                                    onClick={(e) => e.stopPropagation()}
                                                    className="flex-1 flex flex-col justify-center items-center gap-0.5 py-2 min-w-0 text-[10px] font-bold text-green-600 hover:bg-slate-100 transition-all cursor-pointer select-none active:scale-95 border-r border-slate-100"
                                                >
                                                ...
                                                <Link
                                                    ...
                                                    onClick={(e) => e.stopPropagation()}
                                                    className="flex-1 flex flex-col justify-center items-center gap-0.5 py-2 min-w-0 text-[10px] font-bold text-blue-600 hover:bg-slate-100 transition-all cursor-pointer select-none active:scale-95 border-r border-slate-100"
                                                >
