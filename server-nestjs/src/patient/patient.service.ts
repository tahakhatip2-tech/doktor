...
export class PatientService {
            ...
            where: {
                ...
                NOT: {
                    settings: {
                        some: {
                            key: 'clinic_category',
                            value: { in: ['beauty_center', 'home_care', 'lab'] },
                        },
                    },
                },
            },
    ...
    async getLaboratories() {
        const users = await this.prisma.user.findMany({
            where: {
                status: 'active',
                settings: {
                    some: { key: 'clinic_category', value: 'lab' },
                },
            },
            select: {
                id: true,
                ...
                settings: {
                    where: {
                        key: { in: ['clinic_name', 'clinic_logo', 'clinic_cover', 'clinic_description', 'address', 'phone', 'location_url', 'lat', 'lng'] },
                    },
                    select: { key: true, value: true },
                },
                clinicReviews: {
                ...
                services: {
                    where: { isActive: true, category: 'laboratory' },
                    select: { id: true, name: true, price: true, duration: true },
                    take: 6,
                },
            },
    ...
    async getLaboratoryById(providerId: number) {
        const provider = await this.prisma.user.findUnique({
            where: { id: providerId },
            select: {
                id: true,
        ...
        if (!provider) throw new NotFoundException('المختبر غير موجود');
        ...
        return {
            ...
            working_hours,
            clinic_name: settingsMap['clinic_name'] || provider.clinic_name,
            clinic_specialty: settingsMap['clinic_specialty'] || 'مختبرات طبية',
            clinic_logo: settingsMap['clinic_logo'] || null,
            clinic_cover: settingsMap['clinic_cover'] || null,
            clinic_description: settingsMap['clinic_description'] || null,
            location_url: settingsMap['location_url'] || null,
